...
float ID(Mat &mat, int R, int Rn, int K, float xu = 0.0, float yu = 0.0)
{
     ...
     for (int i = 0; i < Rn; i++)
     {
          variance += pow((mat.at<float>(i, 0) + mat.at<float>(i, 1) - xu - yu), 2);
          if (i % 100 == 0)
          {
     ...
     for (int i = 0; i < Rn; i++)
     {
          ID += log(Rn / (root2pi * variance * R)) - (pow(pow(mat.at<float>(i, 0) - xu, 2) + pow(mat.at<float>(i, 1) - yu, 2), 0.5) / (2 * variance));
     }
     return ID;
}

int xmeans(Mat &mat, int R)
{
...
int main(int argc, const char **argv)
{
     ...
     int K = xmeans(samples, whitepixel);
     printf("K = %d\n", K);
     //std::cout << "size: " << centers.size() << endl;
     //std::cout << "centers: " << centers << endl;
     //std::cout << "labels: " << labels.size() << endl;
